...
* an issue with traits when used in combination with stubs has been fixed
but traits produced by earlier versions of Groovy will not have the fix even when
applied using Groovy 2.5.1. The solution is to recompile the trait using 2.5.1.
(link:https://issues.apache.org/jira/browse/GROOVY-8233[GROOVY-8233])
...
If you need the JAXB extension methods, please add that module to your dependency list,
and depending on your JDK version, you may need to add additional dependencies or
command-line switches, or set one of the JAVA_OPTS or JDK_JAVA_OPTIONS environment variables.
[cols="2a,2a"]
|===
|JDK Version |Options
| JDK8 | Do nothing
...
|===
Note: The above description isn't our final plan in this space. For 2.5.1, we focused just on making
...
* remove groovy-jaxb jar from your Groovy distribution
* copying the startGroovy or startGroovy.bat script files from master
* copying the above four files into your lib directory
